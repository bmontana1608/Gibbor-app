...
-- 4. Campo fecha_ingreso_club en perfiles (para fix de deuda histórica)
-- Este campo permite definir explícitamente cuándo ingresó el jugador al club,
-- independientemente de cuándo se creó su cuenta en el sistema.
ALTER TABLE perfiles 
  ADD COLUMN IF NOT EXISTS fecha_ingreso_club DATE;

-- Comentario explicativo
COMMENT ON COLUMN perfiles.fecha_ingreso_club IS 
  'Fecha en que el jugador se unió al club. Si está definida, se usa para calcular deuda histórica en lugar de created_at. Esto evita que jugadores nuevos acumulen deuda de meses anteriores.';
