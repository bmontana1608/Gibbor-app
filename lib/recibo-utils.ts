...
export async function generarReciboNominaPDFBase64(datos: {
  ...
  doc.text('DE EGRESO', 177.5, 26, { align: 'center' });
  doc.setFontSize(7);
  doc.setFont("helvetica", "normal");
  ...
  doc.text([datos.empresa.direccion, datos.empresa.ciudad].filter(Boolean).join(' • '), 45, 28);
  ...
  doc.setTextColor(slateOscuro[0], slateOscuro[1], slateOscuro[2]);
  doc.setFont("helvetica", "normal");
  doc.text(datos.concepto || `Honorarios / Salario - ${datos.periodo}`, 20, tableY + 18);
  doc.setFont("helvetica", "bold");
  ...
  if (datos.firma_recibe) {
    try {
      doc.addImage(datos.firma_recibe, 'PNG', 32, footerY - 25, 40, 20);
    } catch(e) {}
  }
  ...
  doc.setFontSize(7);
  doc.setTextColor(slateGris[0], slateGris[1], slateGris[2]);
  doc.text('FIRMA QUIEN RECIBE', 52, footerY + 5, { align: 'center' });

  // Línea firma director
  if (datos.firma_director) {
    try {
      doc.addImage(datos.firma_director, 'PNG', 132, footerY - 25, 40, 20);
    } catch(e) {}
  }
  doc.line(120, footerY, 185, footerY);
  doc.text('APROBADO POR / DIRECCIÓN', 152, footerY + 5, { align: 'center' });
