...
export async function generarReciboPDFBase64(datos: {
  ...
  if (datos.empresa.logo_url) {
    logoBase64 = await urlToBase64(datos.empresa.logo_url);
  }

  if (logoBase64) {
    try {
      const format = (logoBase64.includes('image/jpeg') || logoBase64.includes('image/jpg')) ? 'JPEG' : 'PNG';
      doc.addImage(logoBase64, format, 15, 8, 24, 24);
    } catch (e) {
      dibujarBadgeInicial(doc, datos.empresa.nombre_club);
    }
  } else {
    dibujarBadgeInicial(doc, datos.empresa.nombre_club);
  }
  ...
  doc.setFont("helvetica", "bold");
  doc.setFontSize(18);
  doc.text(datos.empresa.nombre_club || 'CLUB DEPORTIVO', 45, 22);
...
export async function generarReciboNominaPDFBase64(datos: {
  ...
  // Logo e identidad
  let logoNominaBase64: string | null = null;
  if (datos.empresa.logo_url) {
    logoNominaBase64 = await urlToBase64(datos.empresa.logo_url);
  }

  if (logoNominaBase64) {
    try {
      const format = (logoNominaBase64.includes('image/jpeg') || logoNominaBase64.includes('image/jpg')) ? 'JPEG' : 'PNG';
      doc.addImage(logoNominaBase64, format, 15, 8, 24, 24);
    } catch (e) {
      dibujarBadgeInicial(doc, datos.empresa.nombre_club);
    }
  } else {
