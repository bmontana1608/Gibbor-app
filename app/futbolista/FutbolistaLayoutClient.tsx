...
export default function FutbolistaLayoutClient({ children, initialTenant, initialProfile, initialFamily }: FutbolistaLayoutClientProps) {
  ...
  const menu = [
    { name: t('futbolista.layout.menu.panel'), path: `${basePath}/futbolista`, icon: <Home className="w-5 h-5" /> },
    { name: t('futbolista.layout.menu.pagos'), path: `${basePath}/futbolista/pagos`, icon: <CreditCard className="w-5 h-5" /> },
    // { name: "Partidos Live", path: `${basePath}/futbolista/partidos`, icon: <Play className="w-5 h-5" /> }, // Temporalmente oculto
    { name: t('futbolista.layout.menu.carnet'), path: `${basePath}/futbolista/carnet`, icon: <Award className="w-5 h-5" /> },
    { name: t('futbolista.layout.menu.seguridad'), path: `${basePath}/futbolista/perfil`, icon: <ShieldCheck className="w-5 h-5" /> },
  ];
      ...
      `}>
        <div className="p-8 flex flex-col h-full">
          <div className="flex items-center justify-between mb-12">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-brand text-[10px] font-black uppercase tracking-widest leading-none mt-1">{t('futbolista.layout.playerMode')}</p>
              </div>
            </div>
          ...
          <div className="bg-slate-800/50 rounded-2xl p-4 mb-4 border border-slate-700/50">
             <div className="flex items-center gap-3">
                ...
                <div className="overflow-hidden">
                  ...
                  <p className="text-slate-500 text-[10px] font-medium uppercase tracking-wider">{usuario?.grupos || t('futbolista.layout.athlete')}</p>
                </div>
             </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-8">
            {hijos.length > 1 && (
              <div className="mb-10 px-2">
                <p className="text-slate-500 text-[9px] font-black uppercase tracking-widest mb-3 ml-2">{t('futbolista.layout.yourFamily')}</p>
            ...
            <div className="p-4 bg-slate-950/50 rounded-[1.5rem] border border-slate-800 shadow-xl relative overflow-hidden">
               <p className="text-[9px] font-black text-slate-500 uppercase tracking-widest mb-3 flex items-center gap-2">
                  <ArrowRightLeft className="w-3 h-3" /> {t('futbolista.layout.otherSpaces')}
               </p>
               <div className="space-y-2">
                  {initialProfile?.rol === 'Entrenador' && (
                    ...
                    >
                        ...
                        <span className="text-xs font-bold text-white group-hover:text-emerald-400 transition-colors">{t('futbolista.layout.staffMode')}</span>
                    </Link>
                  )}
                  ...
                  {initialProfile?.rol === 'Director' && (
                    ...
                    >
                        ...
                        <span className="text-xs font-bold text-white group-hover:text-cyan-400 transition-colors">{t('futbolista.layout.directorPanel')}</span>
                    </Link>
                  )}
               </div>
            </div>
          ...
          <div className="space-y-4 pt-6 border-t border-slate-800/50 mt-6 shrink-0">
            ...
            >
              <LogOut className="w-5 h-5" /> {t('futbolista.layout.logout')}
            </button>
          </div>
        </div>
      </aside>
      ...
      <main ref={mainRef} className="flex-1 overflow-y-auto bg-[#F8FAFC] md:ml-72">
        <div className="sticky top-0 bg-white/80 backdrop-blur-md border-b border-slate-100 px-4 md:px-10 py-4 flex items-center justify-between z-40">
           <div className="flex flex-col">
              <h2 className="text-sm font-black text-slate-800 uppercase tracking-tighter italic">{t('futbolista.layout.controlCenter')}</h2>
              <p className="text-[10px] text-slate-400 font-bold uppercase tracking-widest">{usuario?.nombres ? t('futbolista.layout.playerHeader', { name: usuario.nombres }) : t('futbolista.layout.loading')}</p>
           </div>
           <div className="flex items-center gap-3">
              <ThemeToggle />
