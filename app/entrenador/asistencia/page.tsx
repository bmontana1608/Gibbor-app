          ...
          <div className="flex items-center gap-2 mb-1">
             {paso !== 'categorias' && (
               <button 
                 ...
                 className="p-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-500 hover:-[var(--brand-primary)] transition-all mr-2"
               >
             ...
             <h1 className="text-2xl font-black text-slate-900 dark:text-white uppercase italic tracking-tighter flex items-center gap-3">
               <ClipboardCheck className="w-8 h-8 -[var(--brand-primary)]" /> Control Táctico
             </h1>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {categorias.map(cat => (
              <button 
                ...
                className="bg-white dark:bg-slate-900 p-8 rounded-[2.5rem] border border-slate-200 dark:border-slate-800 shadow-sm hover:shadow-2xl hover:-[var(--brand-primary)]/50 hover:-translate-y-1 transition-all text-left relative group overflow-hidden"
              >
                <div className="absolute top-0 right-0 w-32 h-32 -[var(--brand-primary)]/5 rounded-full -translate-y-1/2 translate-x-1/2 group-hover:-[var(--brand-primary)]/10 transition-colors" />
                
                <div className="flex flex-col gap-4 relative z-10">
                  <div className="w-16 h-16 -[rgba(var(--brand-primary-rgb),0.1)] dark:-[var(--brand-primary)]/10 rounded-2xl flex items-center justify-center -[var(--brand-primary)] font-black text-2xl shadow-inner -[var(--brand-primary)]/5">
                    {cat.nombre.charAt(0)}
                  </div>
                  <div>
                    ...
                    <div className="flex items-center gap-2 mt-4 text-slate-500 text-xs font-bold">
                       <Clock className="w-3.5 h-3.5 -[var(--brand-primary)]" /> {cat.horarios || 'Horario flexible'}
                    </div>
                  </div>
                </div>
              </button>
        ...
        {paso === 'eventos' && (
          <div className="space-y-6">
            ...
            <button
              ...
              disabled={cargando}
              className="w-full bg-slate-900 dark:-[var(--brand-primary)] text-white px-8 py-5 rounded-[2rem] font-black uppercase text-sm tracking-widest hover:scale-[1.02] active:scale-95 transition-all shadow-xl flex items-center justify-center gap-3"
            >
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {eventos.map(ev => (
                    <button
                      ...
                      className="bg-white dark:bg-slate-900 p-6 rounded-[2rem] border border-slate-200 dark:border-slate-800 flex items-center gap-5 hover:-[var(--brand-primary)] shadow-sm transition-all text-left"
                    >
                      <div className={`w-14 h-14 rounded-2xl flex items-center justify-center flex-shrink-0 ${ev.tipo === 'Partido' ? 'bg-indigo-100 text-indigo-600' : '-[rgba(var(--brand-primary-rgb),0.1)] -[var(--brand-primary)]'}`}>
          ...
          <div className="bg-white dark:bg-slate-900 rounded-[3rem] border border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden">
               ...
               <div className="flex items-center justify-between gap-4 flex-wrap">
                  <div className="flex items-center gap-4">
                    <div className="w-12 h-12 bg-white dark:bg-slate-800 rounded-2xl flex items-center justify-center shadow-sm">
                      <Users className="w-6 h-6 -[var(--brand-primary)]" />
                    </div>
                    <div>
                  ...
                  <div className="flex items-center gap-2 ml-auto">
                    <button 
                      ...
                      className={`px-3 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest border transition-all ${mostrarNoAutorizados ? '-[var(--brand-primary)] text-white -[var(--brand-primary)]' : 'bg-white dark:bg-slate-800 text-slate-400 border-slate-200 dark:border-slate-700'}`}
                    >
                    ...
                    <div className="relative flex-1 max-w-xs">
                      ...
                      <input 
                        ...
                        className="w-full pl-10 pr-4 py-2 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl text-xs outline-none focus:ring-2 focus:-[var(--brand-primary)] transition-all font-medium"
                      />
                    </div>
                  </div>
               </div>
                ...
                filtrados.map((alumno) => (
                  <div key={alumno.id} className="p-4 md:p-6 flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                       ...
                       <div>
                         <p className="font-black text-slate-800 dark:text-white uppercase italic text-sm md:text-base group-hover:-[var(--brand-primary)] transition-colors">{alumno.nombres} {alumno.apellidos}</p>
            ...
            <div className="p-8 bg-slate-50 dark:bg-slate-900 border-t border-slate-100 dark:border-slate-800 flex flex-col md:flex-row items-center justify-between gap-6">
               ...
               <button 
                 ...
                 className="w-full md:w-auto bg-slate-900 dark:-[var(--brand-primary)] text-white px-12 py-5 rounded-[1.5rem] font-black uppercase text-xs tracking-widest shadow-2xl -[var(--brand-primary)]/20 hover:scale-[1.03] active:scale-95 transition-all flex items-center justify-center gap-3 disabled:opacity-50"
               >
