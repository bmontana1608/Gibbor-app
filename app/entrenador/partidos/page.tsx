    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="mb-8 border-b pb-6">
        <h1 className="text-3xl font-black text-slate-800 uppercase italic tracking-tighter flex items-center gap-3">
          ...
          {t('entrenador.partidos.liveStreams')}
        </h1>
        <p className="text-slate-500 font-medium">{t('entrenador.partidos.subtitle')}</p>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {partidos.map(p => (
          <div key={p.id} className="bg-white rounded-3xl p-6 border border-slate-100 shadow-sm flex items-center justify-between hover:shadow-md transition-shadow group">
            <div>
              <span className="text-[10px] font-black uppercase text-white px-2 py-1 rounded mb-2 inline-block" style={{ backgroundColor: brandColor }}>{p.estado_partido || t('entrenador.partidos.notStarted')}</span>
        ...
        {partidos.length === 0 && (
          <p className="text-slate-400 font-bold p-8 text-center col-span-2">{t('entrenador.partidos.noMatches')}</p>
        )}
      </div>
    </div>
  );
