...
export default function Ranking() {
  ...
  if (cargando && ranking.length === 0) return <div className="p-20 text-center"><Loader className="text-brand" /></div>;
  ...
  return (
    <div className="p-4 md:p-8 max-w-7xl mx-auto space-y-8 pb-20">
      ...
      <div className="relative bg-slate-900 rounded-[40px] p-8 md:p-12 overflow-hidden shadow-2xl">
         <div className="bg-brand/20 to-transparent"></div>
                                    ...
                                    <tr key={jugador.id} className={`group hover:bg-brand/10/30 transition-all ${pos <= 3 ? 'bg-slate-50/20' : ''}`}>
                                        ...
                                        <td className="p-6">
                                            <div className="flex items-center gap-4">
                                                <div className="text-brand transition-colors">
                                        ...
                                        <td className="p-6 text-right">
                                            <div className="flex items-center justify-end gap-2">
                                                <Star className={`w-4 h-4 ${pos === 1 ? 'text-brand text-brand' : 'text-slate-200'}`} />
                                                <span className={`font-black text-xl ${pos === 1 ? 'text-brand' : 'text-slate-800'}`}>{jugador.puntos || 0}</span>
                                            </div>
                                        </td>
                                    </tr>
                                );
         ...
         <div className="space-y-8">
            <div className="bg-slate-900 rounded-[40px] p-8 text-white shadow-2xl relative overflow-hidden group">
                 ...
                 <div className="space-y-6">
                    {recientes.map((log) => (
                        <div key={log.id} className="relative pl-6 border-l-2 text-brand/30 py-1">
