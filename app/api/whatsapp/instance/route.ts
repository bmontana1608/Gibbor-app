...
export async function DELETE(request: Request) {
  try {
    ...
    const logoutRes = await fetch(`${cleanUrl}/instance/logout/${instanceName}`, {
      method: 'DELETE',
      headers: { 'apikey': EVOLUTION_API_KEY! }
    });
    ...
    const deleteRes = await fetch(`${cleanUrl}/instance/delete/${instanceName}`, {
      method: 'DELETE',
      headers: { 'apikey': EVOLUTION_API_KEY! }
    });
    const deleteTxt = await deleteRes.text().catch(() => '');
    
    return NextResponse.json({ 
      success: true, 
      message: 'Intento de desconexión enviado',
      logout_status: logoutRes.status,
      logout_response: logoutTxt,
      delete_status: deleteRes.status,
      delete_response: deleteTxt
    });
  } catch (error: any) {
    return NextResponse.json({ error: error.message }, { status: 500 });
  }
