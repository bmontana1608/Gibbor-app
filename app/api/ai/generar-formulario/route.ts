...
import { callGeminiWithRotation } from '@/lib/gemini-pool';
...
`;
    ...
    // Llamada con rotación inteligente del pool de hasta 10 claves
    const { text: rawText, keyIndexUsed, totalKeys } = await callGeminiWithRotation({
      contents: [
    ...
    console.log(`[GenerarFormulario] Generado con éxito usando clave #${keyIndexUsed + 1}/${totalKeys}`);

    // Parsear JSON limpiando posibles etiquetas markdown
    let parsedForm;
    try {
    ...
    return NextResponse.json({
      ...
      meta: {
        claveUsada: keyIndexUsed + 1,
        totalClavesPool: totalKeys
      }
    });
