...
export async function GET(request: Request) {
  ...
  // 1. Obtenemos el perfil primero (necesario para el filtro de eventos)
    ...
    const [
      ...
      // Pagos
      supabaseAdmin.from("pagos_ingresos").select("*").eq("jugador_id", id).eq('club_id', (tenant as any).id).order("fecha", { ascending: false }).limit(6),
      
      // Asistencias
      supabaseAdmin.from("asistencias").select("*").eq("jugador_id", id).eq('club_id', (tenant as any).id).order("fecha", { ascending: false }),
