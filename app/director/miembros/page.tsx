      ...
      <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 dark:text-white flex items-center gap-2">
            <Users className="-[var(--brand-primary)] w-8 h-8" /> Gestión de Miembros
          </h1>
        ...
        <div className="flex items-center gap-3">
          <button onClick={cargarJugadores} className="p-2.5 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-xl hover:bg-slate-50 transition-all shadow-sm" title="Refrescar datos">
            <Activity className={`w-4 h-4 -[var(--brand-primary)] ${cargando ? 'animate-spin' : ''}`} />
          </button>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-[2rem] border border-slate-200 dark:border-slate-800 shadow-sm overflow-hidden mb-8">
        <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex flex-col lg:flex-row gap-4 items-center justify-between bg-slate-50/50 dark:bg-slate-900/50">
          <div className="flex bg-slate-200/50 dark:bg-slate-800 p-1 rounded-xl w-full lg:w-auto">
            <button onClick={() => setPestaña('Registrados')} className={`flex-1 lg:flex-none px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all ${pestaña === 'Registrados' ? 'bg-white dark:bg-slate-700 -[var(--brand-primary)] shadow-sm' : 'text-slate-500'}`}>Miembros</button>
            <button onClick={() => setPestaña('Pendientes')} className={`flex-1 lg:flex-none px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all relative ${pestaña === 'Pendientes' ? 'bg-white dark:bg-slate-700 -[var(--brand-primary)] shadow-sm' : 'text-slate-500'}`}>
              Solicitudes
              {jugadores.filter(j => (j.estado_miembro || '') !== 'Activo' && (j.estado_miembro || '') !== 'Inactivo').length > 0 && (
                <span className="absolute -top-1 -right-1 w-5 h-5 -[var(--brand-primary)] text-white text-[10px] font-black rounded-full flex items-center justify-center animate-bounce shadow-lg border-2 border-white dark:border-slate-700">
                  {jugadores.filter(j => (j.estado_miembro || '') !== 'Activo' && (j.estado_miembro || '') !== 'Inactivo').length}
                </span>
              )}
            </button>
            <button onClick={() => setPestaña('Cumpleaños')} className={`flex-1 lg:flex-none px-6 py-2 rounded-lg text-xs font-black uppercase tracking-widest transition-all relative flex items-center gap-2 ${pestaña === 'Cumpleaños' ? 'bg-white dark:bg-slate-700 -[var(--brand-primary)] shadow-sm' : 'text-slate-500'}`}>
              ...
              {jugadores.filter(j => {
                ...
                <span className="ml-1 px-1.5 py-0.5 -[var(--brand-primary)] text-white text-[9px] font-black rounded-md">
                  {jugadores.filter(j => {
                    if (!j.fecha_nacimiento || j.estado_miembro !== 'Activo') return false;
                    return (new Date(j.fecha_nacimiento).getUTCMonth() + 1) === (new Date().getMonth() + 1);
                  }).length}
          ...
          <div className="flex flex-col md:flex-row items-center gap-3 w-full lg:w-auto flex-1 lg:justify-end">
            <div className="relative w-full md:max-w-md">
              ...
              <input type="text" placeholder="Buscar por nombre o documento..." value={busqueda} onChange={(e) => setBusqueda(e.target.value)} className="w-full pl-10 pr-4 py-2.5 bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl outline-none focus:ring-2 focus:-[var(--brand-primary)] text-sm transition-all" />
            </div>
            <select value={filtroGrupo} onChange={(e) => setFiltroGrupo(e.target.value)} className="w-full md:w-auto bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-4 py-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:-[var(--brand-primary)] font-medium">
              {gruposDisponibles.map(g => <option key={g} value={g}>{g}</option>)}
            </select>
            <select value={filtroEdad} onChange={(e) => setFiltroEdad(e.target.value)} className="w-full md:w-auto bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 px-4 py-2.5 rounded-xl text-sm outline-none focus:ring-2 focus:-[var(--brand-primary)] font-medium">
              ...
              <div className="mt-8 p-6 bg-slate-50 dark:bg-slate-800/50 rounded-3xl border border-slate-100 dark:border-slate-800 relative group">
                ...
                <div className="flex items-center gap-3">
                  <div className="flex-1 bg-white dark:bg-slate-900 p-4 rounded-xl border border-slate-200 dark:border-slate-700 text-xs font-mono text-slate-600 dark:text-slate-400 truncate">
                    {typeof window !== 'undefined' ? `${window.location.origin}/${tenant?.slug || 'gibbor'}/unete-gibbor?invite=true` : ''}
                  </div>
                  <button onClick={() => { 
                    const link = `${window.location.origin}/${tenant?.slug || 'gibbor'}/unete-gibbor?invite=true`;
                    navigator.clipboard.writeText(link); 
                    toast.success("Copiado!"); 
                  }} className="p-4 bg-[var(--brand-primary)] text-white rounded-xl shadow-lg shadow-[var(--brand-primary)]/20 transition-all">
