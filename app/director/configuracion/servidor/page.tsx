...
export default function ConfiguracionServidor() {
  ...
  useEffect(() => {
    async function loadConfig() {
      ...
      try {
        ...
        const clubId = tenantData?.id;

        let query = supabase.from('configuracion_wa').select('*');
        if (clubId) {
        ...
        if (data) {
          ...
          setConfig(prev => ({ ...prev, club_id: clubId || '' }));
        }
      } catch (e) {
        console.error("Error al cargar configuracion_wa servidor:", e);
      } finally {
        setLoadingConfig(false);
      }
    }
  ...
  const handleSave = async () => {
    setCargando(true);
    let query = supabase.from('configuracion_wa').select('id');
    if (config.club_id) {
      query = query.eq('club_id', config.club_id);
    }
    const { data: existing } = await query.maybeSingle();
    
    const payload: any = { ...config, updated_at: new Date() };
