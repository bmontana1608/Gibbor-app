...
export default function FormulariosDirectorPage() {
  ...
  const cargarFormularios = async (clubId: string) => {
    try {
      const res = await fetch(/api/formularios?clubId=);
  ...
  const eliminarFormulario = async (id: string, titulo: string) => {
    if (!confirm(¿Estás seguro de eliminar el formulario \"\"? También se eliminarán todas sus respuestas asociadas.)) {
      return;
    }

    setEliminandoId(id);
    try {
      const res = await fetch(/api/formularios/, { method: 'DELETE' });
      const data = await res.json();
                ...
                {cargando ? (
                  ...
                  formularios.map(form => (
                    <tr key={form.id} className=\"hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors group\">
                      <td className=\"p-5\">
                        <Link href={route(/director/formularios/)} className=\"font-black text-slate-900 dark:text-white hover:text-brand transition-colors text-sm block\">
                          {form.titulo}
                        </Link>
                        {form.descripcion && (
                      ...
                      <td className=\"p-5 text-center\">
                        <Link
                          href={route(/director/formularios/?tab=respuestas)}
